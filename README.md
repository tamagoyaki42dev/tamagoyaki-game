# Perrant（ペラント） — たまごやきゲームズ

> 完全開示（オープン開発）スタイルで作るインディーゲーム。コードも進捗も全部公開。

## ゲームについて

モールモースの騎兵隊 / ヴィーナス＆ブレイブス にインスパイアされた、  
**ローテーションバトル × 世代継承 × ローグライク**。

### コアコンセプト

- **面白い戦闘** — 原作（モールモース/V&B）のローテーションバトルを核とする
- **投資が形になる** — 時間をかけたものが強さになり蓄積する。世代継承でキャラへの愛着も積み上がる。負けは終わりでなく次世代への布石（滅亡→再興のローグライクループで包む）
- **引き算の美学** — マップなし・属性なし。画面遷移は拠点・編成・戦闘の3つだけ

### プレイ環境

- PC（デスクトップ）横画面

## 現在の状態

**プロトタイプ1リリース済み**（itch.ioで公開中）。敵情報画面 → 編成 → 戦闘 → 3戦クリアまでの基本ループが遊べます。次はプロト2（継承・育成）へ。

## 技術スタック

- **エンジン**: Godot 4 (GDScript)
- **データ**: Godot Resource（完全オフライン）
- **自動テスト**: GUT
- **公開**: itch.io（Windowsデスクトップ版＋ブラウザで遊べるWeb体験版）／ Steam（将来・デスクトップ版）

## 開発ログ

[devlog/](./devlog/) — 毎セッションの進捗を記録。開発過程を全部公開しています。

## 開発スタイル

[Claude Code](https://claude.ai/code) と人間（CEO: たまごやき）の協働開発。  
ゲームデザイン・意思決定は人間、コード実装はほぼ Claude Code が担当。  
コードもプロンプトも全部 GitHub で公開。

## リンク

- X: [@tamagoyaki42dev](https://x.com/tamagoyaki42dev)
- Bluesky: [@tamagoyaki42dev.bsky.social](https://bsky.app/profile/tamagoyaki42dev.bsky.social)
- YouTube: [@たまごやきgames](https://www.youtube.com/@たまごやきgames)
- TikTok: [@tamagoyaki42dev](https://www.tiktok.com/@tamagoyaki42dev)
- itch.io: [tamagoyakigames](https://tamagoyaki42dev.itch.io/tamagoyakigames)

## Credits

- **BGM:** Eric Matyas (Soundimage.org)
- **3Dキャラクター:** Kenney (kenney.nl) — CC0
- **モンスター:** Quaternius (quaternius.com) — CC0
- **武器:** Quaternius RPG Items — CC0

---

*by たまごやきゲームズ*
