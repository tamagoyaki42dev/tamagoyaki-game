...
func test_rotate_show_duration_default() -> void:
	var scene := BattleScene.new()
	assert_almost_eq(scene.rotate_show_duration, 0.35, 0.001,
		"rotate_show_duration のデフォルト値が 0.35 である")
	scene.free()

func test_total_rotate_wait_matches_old_delay() -> void:
	var scene := BattleScene.new()
	var total := scene.rotate_duration + scene.rotate_show_duration
	assert_almost_eq(total, 0.90, 0.001,
		"rotate_duration + rotate_show_duration = 0.90s（旧 ROTATE_DELAY と同じ）")
	scene.free()
