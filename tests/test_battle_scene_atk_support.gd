...
# _make_support_label は static func なので
...
func test_label_text() -> void:
	var lbl := BattleScene._make_support_label(
		"ATK Up", Vector3.ZERO, 48, 0.012, Color(1.0, 0.65, 0.1), _DEFAULT_OFFSET)
...
func test_label_color() -> void:
	...
	var lbl := BattleScene._make_support_label(
		"ATK Up", Vector3.ZERO, 48, 0.012, color, _DEFAULT_OFFSET)
...
func test_label_position_offset() -> void:
	...
	var lbl := BattleScene._make_support_label("ATK Up", base, 48, 0.012, Color.WHITE, offset)
...
func test_def_label_text() -> void:
	var lbl := BattleScene._make_support_label(
		"DEF Up", Vector3.ZERO, 48, 0.012, Color(0.3, 0.7, 1.0), _DEFAULT_OFFSET)
...
func test_def_label_color() -> void:
	...
	var lbl := BattleScene._make_support_label(
		"DEF Up", Vector3.ZERO, 48, 0.012, color, _DEFAULT_OFFSET)
...
func test_def_label_position_offset() -> void:
	var base   := Vector3(1.0, 0.0, 2.0)
	var offset := Vector3(0.0, 1.8, 0.0)
	var lbl := BattleScene._make_support_label("DEF Up", base, 48, 0.012, Color.WHITE, offset)
	assert_almost_eq(lbl.position.y, base.y + offset.y, 0.001, "Y オフセットが加算されている")
	lbl.free()
